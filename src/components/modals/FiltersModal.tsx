...
import { Filter, Priority, StageOptions } from '@/utils/types';
import { generateId, setPriorityColor } from '@/utils/utils';
...
import { ITask, TeamMember } from '@/store/projects/projects.slice';
...
const FiltersModal = ({
  ...
  return (
    <Modal
        ...
        submitBtnText={`Apply filters ${!!selectedFilters.length ? `(${selectedFilters.length})` : ''}`}
        closeBtnText='Close'
        additionalStyles='z-50'
    >
        <form
            onSubmit={applyFilters}
            className={`
